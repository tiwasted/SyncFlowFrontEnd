...
const OrderDetails = () => {
  ...
  return (
    <div className="order-details">
      <h1>Детали заказа</h1>
      <p>
        <strong>Наименование:</strong> {order.order_name}
      </p>
      <p>
      ...
      <p>
        <strong>Время заказа:</strong> {order.order_time}
      </p>
      <p>
      ...
      <p>
        <strong>Отчет:</strong> {order.report || "Нет отчета"}
      </p>
      <p>
        <strong>Сотрудник:</strong>{" "}
        {order.assigned_employee_name || "Не назначен"}
      </p>
      {order.assigned_employee_name && (
        <div>
          <p>
            <strong>Телефон:</strong> {order.assigned_employee_phone}
          </p>
        </div>
      )}
    </div>
